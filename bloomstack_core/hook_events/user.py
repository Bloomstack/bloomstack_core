...
def update_bloomtrace_user(user, method):
	if frappe.conf.developer_mode or frappe.conf.disable_user_sync or user.is_new():
		return
	...
	for request in pending_requests:
		...
		try:
			bloomstack_site_user = frappe_client.get_doc("Bloomstack Site User", filters={
				...
				"email": user.email
			}, fields=['name', 'works_with_bloomstack'])

			if not bloomstack_site_user:
				bloomstack_site_user = insert_bloomstack_site_user(user, get_host_name(), frappe_client)

			elif not bloomstack_site_user[0].get('works_with_bloomstack'):
				doc_name = bloomstack_site_user[0].get('name')
				bloomstack_site_user = update_bloomstack_site_user(user, doc_name, get_host_name(), frappe_client)
...
def insert_bloomstack_site_user(user, site_url, frappe_client):
	return frappe_client.insert(get_bloomstack_site_user(user, site_url))


def update_bloomstack_site_user(user, doc_name, site_url, frappe_client):
	bloomstack_site_user = get_bloomstack_site_user(user, site_url)
...
def get_bloomstack_site_user(user, site_url):
