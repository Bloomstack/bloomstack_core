...
def validate_default_license(doc, method):
	...
	if len(doc.licenses) == 1:
		...
		default_licenses_count = len([license for license in doc.licenses if license.is_default])

		# prevent users from setting multiple default licenses
		if not default_licenses_count:
			frappe.throw(_("There must be atleast one default license, found none."))
		elif default_licenses_count > 1:
			frappe.throw(_("There can be only one default license for {0}, found {1}.").format(doc.name, default_licenses_count))
...
def validate_delivery_window(doc, method):
	...
	if not frappe.db.get_single_value("Delivery Settings", "send_delivery_window_warning") or not (doc.get("delivery_start_time") and doc.get("delivery_end_time")) \
		or not doc.get("customer"):
	...
	if to_timedelta(doc.delivery_start_time) < to_timedelta(delivery_start_time) \
		...
		if method == "validate":
			...
			role_profile_users = [user.email for user in frappe.get_all("User", filters={
					"role_profile_name": ["IN", ["Fulfillment Manager"]]
				}, fields=["email"])]
