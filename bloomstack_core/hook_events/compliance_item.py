...
def _sync_metrc_item(compliance_item):
	...
	if not compliance_item.metrc_id:
		...
		if metrc_id:
			...
			frappe.msgprint(_("{1} was successfully created in METRC.").format(item.item_name))
	else:
		update_item(item)
...
def insert_compliance_item(compliance_item, frappe_client):
	...
	bloomtrace_compliance_item = frappe_client.insert(make_compliance_item(compliance_item))
